...
// PublishRetry routes body to the retry exchange at the given level with an
// updated x-attempt header. level is 1-3.
func (a *RetryPublisherAdapter) PublishRetry(
	...
	if err := a.pub.PublishToExchange(ctx, ExchangeRetry, routingKey, headers, body); err != nil {
		return fmt.Errorf("publish retry level %d for channel %s: %w", level, channel, err)
	}
	return nil
}
